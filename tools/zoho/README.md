...
**Deals — Stage-based field visibility:** source is `../../artifacts/zoho/client_scripts/deal_stage_field_visibility.js`; helper is `provision_deals_stage_visibility_client_script.py` (`make zoho-deals-stage-visibility-client-script`). Public v8/MCP does not reliably create **Developer Hub → Client Script** rows. The working no-manual path is the **Safari admin-session** internal API flow documented in `../../docs/zoho/AUTOMATION-STACK.md` §4.1: `/crm/v2.2/settings/cscript_pages` + `/cscript_snippets`, browser CSRF headers, Zoho's own `compile_script(...)` for `async_code`, and required static resources (`ZRC-1.0`, `Kernel`, `ZDK-1.0`, `DotSDK-2.0`, `Concluder`). For Deals, confirm selector values because Zoho may route the module internally as **Potentials**.
